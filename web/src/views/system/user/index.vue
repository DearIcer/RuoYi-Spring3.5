...
import { deptApi } from "@/api/modules/sys/system/dept";
import { userApi } from "@/api/modules/sys/system/user";
...
const columns = ref({
  ...
  userName: { visible: true, label: "用户名称" },
  nickName: { visible: true, label: "用户昵称" },
  deptName: { visible: true, label: "部门" },
  phonenumber: { visible: true, label: "手机号码" },
...
function getTreeselect() {
  deptApi.treeselect().then(response => {
    ...
    enabledDeptOptions.value = handleTree(response.data, "id");
  });
};
...
function getList() {
  loading.value = true;
  userApi.listUser(addDateRange(queryParams.value, dateRange.value)).then(res => {
...
function handleResetPwd(row) {
  proxy.$prompt('请输入"' + row.userName + '"的新密码', "提示", {
    ...
    userApi.resetUserPwd(row.userId, value).then(response => {
...
function handleAdd() {
  ...
  userApi.deptTreeSelect().then(response => {
    ...
    open.value = true;
    title.value = "添加用户";
    form.value.password = ""; // 使用空密码，后端会生成默认密码
  });
};
